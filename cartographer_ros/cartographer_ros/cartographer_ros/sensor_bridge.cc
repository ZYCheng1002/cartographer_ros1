...
SensorBridge::SensorBridge(const int num_subdivisions_per_laser_scan, const bool ignore_out_of_order_messages,
                           const std::string& tracking_frame, const double lookup_transform_timeout_sec,
  ...
  if (odometry_data != nullptr) {
    if (IgnoreMessage(sensor_id, odometry_data->time)) {
      LOG(WARNING) << "Ignored odometry message from sensor " << sensor_id << " because sensor time "
                   << odometry_data->time << " is not before last odometry message time "
...
void SensorBridge::HandleImuMessage(const std::string& sensor_id, const sensor_msgs::Imu::ConstPtr& msg) {
  ...
  if (imu_data != nullptr) {
    ...
    if (IgnoreMessage(sensor_id, imu_data->time)) {
      LOG(WARNING) << "Ignored IMU message from sensor " << sensor_id << " because sensor time " << imu_data->time
                   << " is not before last IMU message time " << latest_sensor_time_[sensor_id];
      return;
    }
...
void SensorBridge::HandleRangefinder(const std::string& sensor_id, const carto::common::Time time,
  ...
  if (sensor_to_tracking != nullptr) {
    if (IgnoreMessage(sensor_id, time)) {
      LOG(WARNING) << "Ignored Rangefinder message from sensor " << sensor_id << " because sensor time " << time
                   << " is not before last Rangefinder message time " << latest_sensor_time_[sensor_id];
      return;
    }
    latest_sensor_time_[sensor_id] = time;
    /// 在构建slam系统已经形成(LocalTrajectory-->GlobalTrajectory-->CollatedTrajectory逐个包含),
    /// 因为都继承TrajectoryBuilderInterface,所以都存在AddSensorData,先看CollatedTrajectory
    trajectory_builder_->AddSensorData(
